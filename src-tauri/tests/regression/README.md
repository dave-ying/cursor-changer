...
This directory contains regression tests for previously fixed bugs. Each test ensures that a bug, once fixed, does not reappear in future code changes.
...
- `issue_NNN_description.rs` - For GitHub/issue tracker issues
- `bug_YYYY_MM_DD_description.rs` - For bugs without issue numbers
- `regression_feature_description.rs` - For feature-specific regressions
...
```bash
cargo test --test regression_issue_123
```

Run with verbose output:
```bash
cargo test --test 'regression_*' -- --nocapture
```
